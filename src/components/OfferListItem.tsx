    ...
    <View>
      {showingItems.map((item, index) => {
        return (
          <Pressable
            ...
            <Text style={[common.text_l, common.fwb, { width: '85%' }]} numberOfLines={1}>
              {item.title}
            </Text>
      ...
      {button && (
        <Pressable onPress={loadMore} style={styles.moreButton}>
          <Text style={[common.text_m, common.tac, common.mr8]}>{isMore ? '접기' : '더보기'}</Text>
          <Icon name={isMore ? 'angle-up' : 'angle-down'} size={24} color="black" />
        </Pressable>
      )}
    </View>
  );
